...
document.querySelector('#scoreTable').addEventListener('click', (e) => {
  // 1. 判断点击的是"查看"链接
  const viewLink = e.target.closest('a[href="javascript:;"][data-score-id]');

  if (viewLink) {
    e.preventDefault();

    // 判断是查看还是编辑（根据链接文字）
    const linkText = viewLink.textContent.trim();
    scoreId = viewLink.dataset.scoreId;

    if (linkText === '查看') {

      // 获取当前行数据
      // 从当前点击的链接开始，向上找父元素中第一个 < tr > 标签
    ...
    if (linkText === '编辑') {
      ...
      document.getElementById('editScoreValue').value = cells[4].textContent;
